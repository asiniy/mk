class UserMailer < ActionMailer::Base
  ...
  def category_subscription(user_id, post_id, category_ids)
    ...
    mail to: @user.email, subject: "Опубликована новая статья \"#{@post.heading}\" в #{Russian.p(@categories.length, 'категории', 'категориях', 'категориях')} #{[*@categories.map(&:title)].join(', ')}"
  end
end
